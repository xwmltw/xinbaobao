//
//  GJJTaoBaoCreditViewController.m
//  RenRenhua2.0
//
...
typedef NS_ENUM(NSInteger, GJJTaoBaoCreditRequest) {
    GJJTaoBaoCreditRequestData,
    GJJTaoBaoCreditRequestSubmit,
};

@interface GJJTaoBaoCreditViewController ()
<MBProgressHUDDelegate>

@end

@implementation GJJTaoBaoCreditViewController
{
...
- (void)viewDidLoad {
    ...
    if ([_model.authenStatus integerValue] != 0) {
        [self prepareDataWithCount:GJJTaoBaoCreditRequestData];
    }else {
        [self setupView];
    }
}
...
{
    if (self.requestCount == GJJTaoBaoCreditRequestData) {
        self.cmd = GJJQueryMyCertId;
        self.dict = @{@"userId":[self getSeccsion].userId,
                      @"qryType":@"10"};
    }else if (self.requestCount == GJJTaoBaoCreditRequestSubmit) {
...
- (void)certificationClick:(UIButton *)sender
{
    ...
    [self prepareDataWithCount:GJJTaoBaoCreditRequestSubmit];
}
