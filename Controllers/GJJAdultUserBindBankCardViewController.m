//
//  GJJAdultUserBindBankCardViewController.m
//  RenRenhua2.0
//
...
typedef NS_ENUM(NSInteger, GJJAdultUserBindBankCardRequest) {
    GJJAdultUserBindBankCardRequestData,
    GJJAdultUserBindBankCardRequestBankInfo,
    GJJAdultUserBindBankCardRequestGetCode,
    GJJAdultUserBindBankCardRequestSubmit,
    GJJAdultUserBindBankCardRequestSupportedBank,
    GJJAdultUserBindBankCardRequestSchedule,
};

@interface GJJAdultUserBindBankCardViewController ()
...
@implementation GJJAdultUserBindBankCardViewController
{
...
//    [self setupView];
    ...
    [self prepareDataWithCount:GJJAdultUserBindBankCardRequestData];
}
...
//        [self prepareDataWithCount:GJJAdultUserBindBankCardRequestData];
//    }else{
//        [self prepareDataWithCount:GJJAdultUserBindBankCardRequestSchedule];
//    }
}
...
//        make.height.mas_equalTo(AdaptationWidth(50));
    ...
    [bankView makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(cardNumberView.mas_bottom).offset(@(AdaptationHeight(10)));
        make.left.equalTo(self.view.mas_left).offset(@(AdaptationWidth(20)));
        make.right.equalTo(self.view.mas_right).offset(@(AdaptationWidth(-20)));
        make.height.equalTo(peopleView.mas_height);
    }];
...
//        make.width.equalTo(_getCodeButton.mas_height).multipliedBy(2.8);
//    }];
    ...
    [getVerificationCodeButton makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(codeView.mas_right).offset(@(AdaptationWidth(-20)));
        make.centerY.equalTo(codeView.mas_centerY);
        make.height.equalTo(codeView.mas_height).multipliedBy(6.0/9);
...
- (void)getVerificationCodeClick:(GJJCountDownButton *)sender
{
    ...
    [self prepareDataWithCount:GJJAdultUserBindBankCardRequestGetCode];
}
...
- (void)chooseBank
{
    if (_bankArray.count == 0) {
        [self prepareDataWithCount:GJJAdultUserBindBankCardRequestBankInfo];
    }else {
...
{
    if (self.requestCount == GJJAdultUserBindBankCardRequestData) {
        self.cmd = GJJQueryMyCertId;
        self.dict = @{@"userId":[self getSeccsion].userId,
                      @"qryType":@"4"};
    }else if (self.requestCount == GJJAdultUserBindBankCardRequestBankInfo) {
        self.cmd = GJJQueryBankAndLog;
        self.dict = @{};
    }else if (self.requestCount == GJJAdultUserBindBankCardRequestGetCode) {
        self.cmd = GJJGetValiCodeFromPhone;
        self.dict = @{@"userId":[self getSeccsion].userId,
                      @"phone":_phoneText.text,
                      @"type":@"3"};
    }else if (self.requestCount == GJJAdultUserBindBankCardRequestSubmit) {
        ...
        self.dict = @{@"realName":[self getSeccsion].customName,
                      ...
                      @"type":@"3"};
    }else if (self.requestCount == GJJAdultUserBindBankCardRequestSupportedBank) {
        self.cmd = GJJQuerySupportedBank;
        self.dict = @{@"bankName":_scanSuccessDict[@"bank_name"]};
    }else if (self.requestCount == GJJAdultUserBindBankCardRequestSchedule){
        ...
        if ([dict[@"isBank"] integerValue]) {
            if (_schedule == 8) {
                ...
                if (getOff) {
                    ...
                    controller.scheduleDict = scheduleDic;
...
- (void)certificationClick:(UIButton *)sender
{
    ...
    [self prepareDataWithCount:GJJAdultUserBindBankCardRequestSubmit];
}
...
- (void)bankCardScanSureInfoWithDict:(NSDictionary *)dict infoView:(GJJBindCardScanSuccessView *)infoView
{
    ...
    [self prepareDataWithCount:GJJAdultUserBindBankCardRequestSupportedBank];
}
