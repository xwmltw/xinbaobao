//
//  GJJUserLinkManViewController.m
//  RenRenhua2.0
//
...
typedef NS_ENUM(NSInteger, GJJUserLinkManRequest) {
    GJJUserLinkManRequestData,
    GJJUserLinkManRequestSaveLinkMan,
};

@interface GJJUserLinkManViewController ()
...
@implementation GJJUserLinkManViewController
{
...
- (void)viewDidLoad {
    ...
    [self setupData];
    [self prepareDataWithCount:GJJUserLinkManRequestData];
}
...
- (void)setupView
{
    ...
    [_parentNameText makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(chooseParentNameButton.left);
    }];
    ...
    if (father.relationship.length != 0) {
       ...
       _parentRelationshipLabel.text = @"父/母";
    }
    ...
    [_parentRelationshipLabel makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerY.equalTo(parentRelationshipView);
        make.right.equalTo(parentRelationshipView.right);
    }];
    ...
    if (father.contactPhone.length != 0) {
       ...
       _parentPhoneLabel.text = @"请选择您父/母的电话号码";
    ...
    if (father.familyAdress.length != 0) {
        ...
        _parentAddressLabel.text = @"请选择您父/母所在的省市区";
    ...
    [placeDetailLabel makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(placeDetailView.left).offset(@10);
    ...
    _familyAddressText.attributedPlaceholder = [[NSAttributedString alloc] initWithString:@"请输入您父/母的具体家庭住址" attributes:@{NSForegroundColorAttributeName: placeholderColor}];
    ...
    if (roommate.relationship.length != 0) {
        ...
        _friendRelationshipLabel.text = @"室友";
    }
    ...
    if (roommate.relationship.length != 0) {
        ...
        _friendPhonePlaceHolder = @"请选择您室友的电话号码";
    }
...
- (void)chooseFriendRelation
{
    ...
    if (!_chooseFriendRelationshipPicker) {
        ...
        _chooseFriendRelationshipPicker.chooseThings = @[@"室友", @"同学", @"朋友", @"辅导员", @"其他"];
    }
...
- (void)chooseCityWithProvince:(NSString *)province city:(NSString *)city town:(NSString *)town chooseView:(GJJChooseCityView *)chooseView
{
    ...
    _parentAddressLabel.text = [NSString stringWithFormat:@"%@ %@ %@", _province, _city, _town];
    _parentAddressLabel.textColor = CCXColorWithHex(@"888888");
    _parentAddressLabel.adjustsFontSizeToFitWidth = YES;
}
...
- (void)certificationClick:(UIButton *)sender
{
    if (_parentNameText.text.length == 0) {
        [self setHudWithName:@"请选择您父/母的姓名" Time:1 andType:1];
        return;
    }
    ...
    if ([_parentAddressLabel.text isEqualToString:@"请选择您父/母所在的省市区"]) {
        [self setHudWithName:@"请选择您父/母所在的省市区" Time:1 andType:1];
        return;
    }
    
    if (_familyAddressText.text.length == 0) {
        [self setHudWithName:@"请输入您父/母的具体家庭住址" Time:1 andType:1];
        return;
    }
    ...
    [self prepareDataWithCount:GJJUserLinkManRequestSaveLinkMan];
}

- (void)setRequestParams
{
    if (self.requestCount == GJJUserLinkManRequestData) {
        self.cmd = GJJQueryMyCertId;
        self.dict = @{@"userId":[self getSeccsion].userId,
                      @"qryType":@"5"};
    }else if (self.requestCount == GJJUserLinkManRequestSaveLinkMan) {
