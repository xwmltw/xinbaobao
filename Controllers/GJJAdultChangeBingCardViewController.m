//
//  GJJAdultChangeBingCardViewController.m
//  RenRenhua2.0
//
...
typedef NS_ENUM(NSInteger, GJJAdultChangeBingCardRequest) {
    GJJAdultChangeBingCardRequestData,
    GJJAdultChangeBingCardRequestBankInfo,
    GJJAdultChangeBingCardRequestGetCode,
    GJJAdultChangeBingCardRequestSubmit,
    GJJAdultChangeBingCardRequestSupportedBank,
};

@interface GJJAdultChangeBingCardViewController ()
...
@implementation GJJAdultChangeBingCardViewController
{
...
- (void)viewDidLoad {
    ...
    [self setupData];
    [self prepareDataWithCount:GJJAdultChangeBingCardRequestData];
}
...
- (void)getVerificationCodeClick:(GJJCountDownButton *)sender
{
    ...
    [self prepareDataWithCount:GJJAdultChangeBingCardRequestGetCode];
}
...
- (void)chooseBank
{
    if (_bankArray.count == 0) {
        [self prepareDataWithCount:GJJAdultChangeBingCardRequestBankInfo];
    }else {
...
{
    if (self.requestCount == GJJAdultChangeBingCardRequestData) {
        self.cmd = GJJQueryMyCertId;
        self.dict = @{@"userId":[self getSeccsion].userId,
                      @"qryType":@"4"};
    }else if (self.requestCount == GJJAdultChangeBingCardRequestBankInfo) {
        self.cmd = GJJQueryBankAndLog;
        self.dict = @{};
    }else if (self.requestCount == GJJAdultChangeBingCardRequestGetCode) {
        self.cmd = GJJGetValiCodeFromPhone;
        self.dict = @{@"userId":[self getSeccsion].userId,
                      @"phone":_phoneText.text,
                      @"type":@"3"};
    }else if (self.requestCount == GJJAdultChangeBingCardRequestSubmit) {
    ...
    }else if (self.requestCount == GJJAdultChangeBingCardRequestSupportedBank) {
...
- (void)certificationClick:(UIButton *)sender
{
    ...
    [self prepareDataWithCount:GJJAdultChangeBingCardRequestSubmit];
}
...
- (void)bankCardScanSureInfoWithDict:(NSDictionary *)dict infoView:(GJJBindCardScanSuccessView *)infoView
{
    ...
    [self prepareDataWithCount:GJJAdultChangeBingCardRequestSupportedBank];
}
