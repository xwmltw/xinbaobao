//
//  GJJAdultJindDongCreditViewController.m
//  RenRenhua2.0
//
...
typedef NS_ENUM(NSInteger, GJJAdultJindDongCreditRequest) {
    GJJAdultJindDongCreditRequestData,
    GJJAdultJindDongCreditRequestSubmit,
};


@interface GJJAdultJindDongCreditViewController ()
<MBProgressHUDDelegate>

@end

@implementation GJJAdultJindDongCreditViewController
{
...
- (void)viewDidLoad {
    ...
    if ([_model.authenStatus integerValue] != 0) {
        [self prepareDataWithCount:GJJAdultJindDongCreditRequestData];
    }else {
        [self setupView];
    }
}
...
{
    if (self.requestCount == GJJAdultJindDongCreditRequestData) {
        self.cmd = GJJQueryMyCertId;
        self.dict = @{@"userId":[self getSeccsion].userId,
                      @"qryType":@"9"};
    }else if (self.requestCount == GJJAdultJindDongCreditRequestSubmit) {
...
- (void)certificationClick:(UIButton *)sender
{
    ...
    [self prepareDataWithCount:GJJAdultJindDongCreditRequestSubmit];
}
