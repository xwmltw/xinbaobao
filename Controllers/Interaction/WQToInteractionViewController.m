...
-(void)requestSuccessWithDictionary:(NSDictionary *)dict{
    MyLog(@"%@",dict);
    self.tableView_Interaction.tableFooterView = nil;
    self.tablelView_System.tableFooterView = nil;
    ...
    for (NSDictionary *interactionDic in dict[@"activAnnounce"]) {
        ...
        if ([user.userId intValue] == -100) {
            interactionAppendingh5Url = interactionModel.h5Url;
            interactionModel.h5Url = interactionAppendingh5Url;
        }else{
            if (interactionModel.h5Url.length > 0) {
                interactionAppendingh5Url = [NSString stringWithFormat:@"%@?%@",interactionModel.h5Url,user.userId];
                interactionModel.h5Url = interactionAppendingh5Url;
            }
        }
        [_interactionArr addObject:interactionModel];
    }
    if (_interactionArr.count == 0) {
        [self setInteractionFooter];
    }
    
    [_rollingPics removeAllObjects];
    NSString *rollingPicsAppendingh5Url = nil;
    for (NSDictionary *rollingPicsDic in dict[@"picAnnounce"]) {
        ...
        if ([user.userId intValue] == -100) {
            rollingPicsAppendingh5Url = picAnnounceModel.h5Url;
            picAnnounceModel.h5Url = rollingPicsAppendingh5Url;
        }else{
            if (picAnnounceModel.h5Url.length > 0) {
                rollingPicsAppendingh5Url = [NSString stringWithFormat:@"%@?%@",picAnnounceModel.h5Url,user.userId];
                picAnnounceModel.h5Url = rollingPicsAppendingh5Url;
            }
        }
...
//    self.headView.backgroundColor = [UIColor colorWithHexString:GJJMainColorString];
    ...
    [self.bottomView setBackgroundColor:[UIColor colorWithHexString:GJJMainColorString]];
    
    [self.buttonA setTitle:@"活动公告" forState:UIControlStateNormal];
    [self.buttonB setTitle:@"系统公告" forState:UIControlStateNormal];
    self.buttonA.titleLabel.font = [UIFont systemFontOfSize:35*CCXSCREENSCALE];
    self.buttonB.titleLabel.font = [UIFont systemFontOfSize:35*CCXSCREENSCALE];
    ...
    [self.buttonA setTitleColor:[UIColor colorWithHexString:GJJMainColorString] forState:UIControlStateSelected];
    [self.buttonB setTitleColor:[UIColor colorWithHexString:GJJMainColorString] forState:UIControlStateSelected];
    
    self.buttonA.titleLabel.font = [UIFont systemFontOfSize:40*CCXSCREENSCALE];
    self.buttonB.titleLabel.font = [UIFont systemFontOfSize:35*CCXSCREENSCALE];
    ...
    _interactionNewImageV.image = [UIImage imageNamed:@"dot"];
    
    _systemNewImageV = [[UIImageView alloc]init];
    _systemNewImageV.image = [UIImage imageNamed:@"dot"];
...
-(void)setRefreshUI{
    for (UIView *view in _headScrollView.subviews) {
    ...
    if (_rollingPics.count) {
        _headScrollView.contentSize = CGSizeMake(CCXSIZE_W*_rollingPics.count, self.headView.frame.size.height);
        
        UIView *lastView = nil;
        
        NSInteger idx = 0;
        for (WQPicAnnounce *model in _rollingPics) {
            UIImageView *imageV = [self CCXImageViewWithImageName:@"activity_banner_default" url:model.picUrl andFrame:CGRectMake(CCXSIZE_W*idx, 0, CCXSIZE_W, _headScrollView.frame.size.height) tag:(int)idx+101];
            idx++;
            ...
            [imageV makeConstraints:^(MASConstraintMaker *make) {
                make.top.equalTo(_headScrollView);
                make.left.equalTo(lastView ? lastView.right : @0);
                make.width.equalTo(_headScrollView);
                make.height.equalTo(_headScrollView);
            }];
            lastView = imageV;
        }
        ...
        self.pageTimer.fireDate = [NSDate date];

    }else {
        _headScrollView.contentSize = CGSizeMake(CCXSIZE_W, self.headView.frame.size.height);
        UIImageView *imageV = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, CCXSIZE_W, _headScrollView.frame.size.height)];
        imageV.image = [UIImage imageNamed:@"activity_banner_default"];
        [_headScrollView addSubview:imageV];
    }
    
    if (_interactionArr.count > 0) {
        for (WQInteractionMessage *interactionModel in _interactionArr) {
            if ([interactionModel.isRead isEqualToString:@"0"]) {
                [self.buttonA addSubview:_interactionNewImageV];
                [_interactionNewImageV mas_makeConstraints:^(MASConstraintMaker *make) {
                    make.top.mas_equalTo(self.buttonA).offset(25*CCXSCREENSCALE);
                    make.left.mas_equalTo(self.buttonA.titleLabel.right);
                    make.width.mas_equalTo(15*CCXSCREENSCALE);
                    make.height.mas_equalTo(15*CCXSCREENSCALE);
                }];
                break;
            }else{
...
- (void)setup_Layout {
    [self.headView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(350*CCXSCREENSCALE);
    }];
    ...
    [self.buttonView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.mas_equalTo(self.view);
        make.width.mas_equalTo(CCXSIZE_W);
        make.height.mas_equalTo(100*CCXSCREENSCALE);
    }];
    ...
    [self.bottomView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(5*CCXSCREENSCALE);
        make.left.mas_equalTo(10*CCXSCREENSCALE);
    }];
    ...
    [self.scrollView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.mas_equalTo(self.view);
        make.width.mas_equalTo(CCXSIZE_W);
        make.height.mas_equalTo(CCXSIZE_H-450*CCXSCREENSCALE-49);
    }];
    ...
    [self.tableView_Interaction mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.mas_equalTo(self.greenView).offset(-20*CCXSCREENSCALE);
    }];
    
    [self.tablelView_System mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.left.width.mas_equalTo(self.orangeView);
        make.bottom.mas_equalTo(self.orangeView).offset(-20*CCXSCREENSCALE);
    }];
}
...
-(NSTimer *)pageTimer{
    if (!_pageTimer) {
        ...
        _pageTimer.fireDate = [NSDate date];
    }
    return _pageTimer;
}
    ...
    if (scrollView == self.scrollView) {
        ...
        if (scrollView.contentOffset.x > CCXSIZE_W / 2) {
            ...
            [self.bottomView mas_updateConstraints:^(MASConstraintMaker *make) {
                make.left.mas_equalTo(20*CCXSCREENSCALE + CCXSIZE_W / 2);
            }];
            //修改button按钮的状态
            [UIView animateWithDuration:0.3 animations:^{
                ...
                self.buttonA.selected = NO;
                self.buttonB.selected = YES;
                self.buttonB.titleLabel.font = [UIFont systemFontOfSize:40*CCXSCREENSCALE];
                self.buttonA.titleLabel.font = [UIFont systemFontOfSize:35*CCXSCREENSCALE];
            }];
        } else {
            [self.bottomView mas_updateConstraints:^(MASConstraintMaker *make) {
                make.left.mas_equalTo(20*CCXSCREENSCALE);
            }];
            [UIView animateWithDuration:0.3 animations:^{
                [self.buttonView layoutIfNeeded];
                self.buttonB.selected = NO;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == self.tableView_Interaction) {
        static NSString *cellID = @"interactionCellId";
...
-(void)setSystemFooter{
    UIView *footerView = [[UIView alloc]initWithFrame:self.tablelView_System.bounds];
    UIImageView *imageV = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"activity_nodata"]];
    [footerView addSubview:imageV];
    [imageV makeConstraints:^(MASConstraintMaker *make) {
        make.center.equalTo(footerView);
        make.size.equalTo(CGSizeMake(AdaptationWidth(150), AdaptationHeight(150)));
    }];
    if (_systemArr.count == 0) {
        self.tablelView_System.tableFooterView = footerView;
    }else {
        self.tablelView_System.tableFooterView = nil;
    }
}
...
-(void)setInteractionFooter{
    UIView *footerView = [[UIView alloc]initWithFrame:self.tableView_Interaction.bounds];
    UIImageView *imageV = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"activity_nodata"]];
    [footerView addSubview:imageV];
    [imageV makeConstraints:^(MASConstraintMaker *make) {
        make.center.equalTo(footerView);
    ...
    if (_interactionArr.count == 0) {
        self.tableView_Interaction.tableFooterView = footerView;
    }else {
        self.tableView_Interaction.tableFooterView = nil;
    }
}
