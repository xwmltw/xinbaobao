...
typedef NS_ENUM(NSInteger, WQInteractionDetailRequest) {
    WQInteractionDetailRequestShare,
    WQQueryOneMessageByMesIdRequest,
};

@interface WQInteractionDetailViewController ()

@end

@implementation WQInteractionDetailViewController
{
    ...
    if (self.requestCount == WQQueryOneMessageByMesIdRequest) {
        ...
        if (self.requestCount == WQQueryOneMessageByMesIdRequest) {
            ...
            if ([user.userId intValue] == -100) {
                self.dict = @{ @"phoneId":uuidStr,@"mesId": self.mesId};
            }else{
                self.dict = @{ @"userId": user.userId,@"phoneId":uuidStr,
                               @"mesId": self.mesId};
            }
//            self.dict = @{ @"userId": [user.userId intValue] == -100 ? @"" : user.userId,@"phoneId":uuidStr,
//                           @"mesId": self.mesId};
         
        };
    }else if (self.requestCount == WQInteractionDetailRequestShare) {
...
- (void)shareClick
{
    ...
    [self prepareDataWithCount:WQInteractionDetailRequestShare];
}
