//
//  GJJAdultIdentityVerifyViewController.m
//  RenRenhua2.0
//
...
typedef NS_ENUM(NSUInteger, GJJAdultIdentityVerifyRequest) {
    GJJAdultIdentityVerifyRequestGetID,
    GJJAdultIdentityVerifyRequestIDCardFrontImage,
    GJJAdultIdentityVerifyRequestIDCardBackImage,
    GJJAdultIdentityVerifyRequestHuoTi,
    GJJAdultIdentityVerifyRequestZM,
    GJJAdultIdentityVerifyRequestZMBack,
};

@interface GJJAdultIdentityVerifyViewController ()
...
@implementation GJJAdultIdentityVerifyViewController
{
    //扫描身份证的页面截图
...
//    [self prepareDataWithCount:GJJAdultIdentityVerifyRequestHuoTi];
//}
//
...
{
    [self setHudWithName:@"用户取消核对" Time:1 andType:2];
}
...
- (void)verifyZM
{
    [self prepareDataWithCount:GJJAdultIdentityVerifyRequestGetID];
}

-(void)setRequestParams
{
    if (self.requestCount == GJJAdultIdentityVerifyRequestGetID) {
        //芝麻信用认证
        self.cmd = GJJQueryMyCertId;
        self.dict = @{@"userId":[self getSeccsion].userId,
                      @"qryType":@"0"};
    }else if (self.requestCount == GJJAdultIdentityVerifyRequestIDCardFrontImage) {
    ...
    }else if (self.requestCount == GJJAdultIdentityVerifyRequestIDCardBackImage) {
        self.cmd = WQUpLoadPicture;
    ...
    }else if (self.requestCount == GJJAdultIdentityVerifyRequestHuoTi) {
...
//        NSString *imgbagString = [[_oliveappDetectedFrame.verificationDataFullWithFanpai base64EncodedStringWithOptions:NSDataBase64EncodingEndLineWithCarriageReturn] stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet punctuationCharacterSet]];
        ...
        self.dict = @{@"userId":[self getSeccsion].userId,
                      @"imgbag":@""};
    }else if (self.requestCount == GJJAdultIdentityVerifyRequestZM) {
        self.cmd = GJJAuthorizeQry;
        self.dict = @{@"userId":[self getSeccsion].userId,
                      @"name":_certIdModel.userName,
                      @"certNo":_certIdModel.identityCard};
    }else if (self.requestCount == GJJAdultIdentityVerifyRequestZMBack) {
...
- (void)requestSuccessWithDictionary:(NSDictionary *)dict
{
    if (self.requestCount == GJJAdultIdentityVerifyRequestGetID) {
        ...
        [self prepareDataWithCount:GJJAdultIdentityVerifyRequestZM];
    }else if (self.requestCount == GJJAdultIdentityVerifyRequestIDCardFrontImage) {
...
{
    if (self.requestCount == GJJAdultIdentityVerifyRequestZM) {
        ...
        if (self.requestCount == GJJAdultIdentityVerifyRequestIDCardFrontImage) {
...
- (void)result:(NSMutableDictionary *)dict
{
    ...
    _zmDict = dict;
    [self prepareDataWithCount:GJJAdultIdentityVerifyRequestZMBack];
}
...
- (void)sureAgainInfo:(NSDictionary *)infoDict infoView:(GJJIDCardScanSuccessView *)infoView
{
    ...
    [self prepareDataWithCount:GJJAdultIdentityVerifyRequestIDCardFrontImage];
}
...
- (void)IDCardScanSureInfoWithInfo:(NSDictionary *)infoDict
{
    if (_IDCardType == 17) {
        ...
        _backDict = infoDict;
        [self prepareDataWithCount:GJJAdultIdentityVerifyRequestIDCardBackImage];
    }
