//
//  GJJLiftingSocialViewController.m
//  RenRenhua2.0
//
...
typedef NS_ENUM(NSInteger, GJJLiftingSocialRequest) {
    GJJLiftingSocialRequestData,
    GJJLiftingSocialRequestSubmit,
};

@interface GJJLiftingSocialViewController ()

@end

@implementation GJJLiftingSocialViewController
{
...
- (void)viewDidLoad {
    ...
    if ([_scheduleDict[@"sjStatus"] integerValue] != 0) {
        [self prepareDataWithCount:GJJLiftingSocialRequestData];
    }else {
        [self setupView];
    }
}
...
- (void)certificationClick:(UIButton *)sender
{
    ...
    [self prepareDataWithCount:GJJLiftingSocialRequestSubmit];
}

- (void)setRequestParams
{
    if (self.requestCount == GJJLiftingSocialRequestData) {
        self.cmd = GJJQueryMyCertId;
        self.dict = @{@"userId":[self getSeccsion].userId,
                      @"qryType":@"8"};
    }else if (self.requestCount == GJJLiftingSocialRequestSubmit) {
