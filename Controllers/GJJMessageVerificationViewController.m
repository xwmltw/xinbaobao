//
//  GJJMessageVerificationViewController.m
//  RenRenhua2.0
//
...
typedef NS_ENUM(NSInteger, GJJMessageVerificationRequest) {
    GJJMessageVerificationRequestMessageCode,
};

typedef NS_ENUM(NSInteger, GJJMessageVerificationSubmitOrResend) {
    GJJMessageVerificationSubmitPassword,
    GJJMessageVerificationResendPassword,
};

@interface GJJMessageVerificationViewController ()

@end

@implementation GJJMessageVerificationViewController
{
    GJJMessageVerificationSubmitOrResend _submitOrResend;
...
- (void)setupData
{
    _submitOrResend = GJJMessageVerificationResendPassword;
    _messageCodeCount = 0;
}
...
- (void)certificationClick:(UIButton *)sender
{
    ...
    _submitOrResend = GJJMessageVerificationSubmitPassword;
    [self prepareDataWithCount:GJJMessageVerificationRequestMessageCode];
}
...
{
    if (self.requestCount == GJJMessageVerificationRequestMessageCode) {
        self.cmd = GJJOperator;
        if (_submitOrResend == GJJMessageVerificationResendPassword) {
            ...
            _messageCodeCount++;
        }else if (_submitOrResend == GJJMessageVerificationSubmitPassword) {
            self.dict = @{
                          @"phone":_phone,
                          @"token":_operatorsModel.token,
                          @"password":_password,
...
//            [self setupTimerAndButtonTitle];
            ...
            }else if ([model.process_code integerValue] == 10002) {
                
                GJJMessageVerificationAgainViewController *controller = [[GJJMessageVerificationAgainViewController alloc]init];
                controller.hidesBottomBarWhenPushed = YES;
                controller.title = @"移动认证";
                controller.operatorsModel = _operatorsModel;
                controller.phone = _phone;
                controller.password = _password;
                controller.popViewController = _popViewController;
                [self.navigationController pushViewController:controller animated:YES];
