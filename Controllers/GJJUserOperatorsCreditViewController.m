//
//  GJJUserOperatorsCreditViewController.m
//  RenRenhua2.0
//
...
typedef NS_ENUM(NSInteger, GJJUserOperatorsCreditRequest) {
    GJJUserOperatorsCreditRequestData,
    GJJUserOperatorsCreditRequestSubmit,
    GJJUserOperatorsCreditRequestMessageCode,
};

@interface GJJUserOperatorsCreditViewController ()
<UITextFieldDelegate>

@end

@implementation GJJUserOperatorsCreditViewController
{
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [self prepareDataWithCount:GJJUserOperatorsCreditRequestData];
}
...
//        }];
//        
//        _scheduleProgressView.progress = 0.875;
//        _scheduleProgressLabel.text = @"87.5%";
//    }
}
...
- (void)certificationClick:(UIButton *)sender
{
    ...
    [self prepareDataWithCount:GJJUserOperatorsCreditRequestMessageCode];
}
...
{
    if (self.requestCount == GJJUserOperatorsCreditRequestData) {
        self.cmd = GJJQueryMyCertId;
        self.dict = @{@"userId":[self getSeccsion].userId,
                      @"qryType":@"3"};
    }else if (self.requestCount == GJJUserOperatorsCreditRequestSubmit) {
        self.cmd = GJJAuthenticationCustom;
        self.dict = @{@"userId":[self getSeccsion].userId,
                      @"authenType":@"2",
                      @"accont":_phoneText.text,
                      @"password":_operatorsText.text};
    }else if (self.requestCount == GJJUserOperatorsCreditRequestMessageCode) {
