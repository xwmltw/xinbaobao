...
//
#import "CCXWaterWaveView.h"
#import "CCXSuperViewController.h"

@interface CCXWaterWaveView ()
...
@implementation CCXWaterWaveView{
    ...
    CGFloat currentWavePoinCCX; // 当前波浪上市高度Y（高度从大到小 坐标系向下增长）
...
- (void)setFrame:(CGRect)frame
{
    ...
    if (currentWavePoinCCX <= 0) {
        currentWavePoinCCX = self.frame.size.height;
    }
}
...
- (void)setUp
{
    waterWaveHeight = self.frame.size.height/2;
    waterWaveWidth  = self.frame.size.width;
//    _firstWaveColor = [UIColor colorWithHexString:@"fdd000"];
    _firstWaveColor = [UIColor colorWithHexString:STFirstWaveColorString];
    _secondWaveColor = [UIColor colorWithHexString:STSecondWaveColorString];
    waveGrowth = 0.85;
    waveSpeed = 0.4/M_PI;
...
-(void)getCurrentWave:(CADisplayLink *)displayLink{
    ...
    if ( waveGrowth > 0 && currentWavePoinCCX > 2 * waterWaveHeight *(1-_percent)) {
        // 波浪高度未到指定高度 继续上涨
        currentWavePoinCCX -= waveGrowth;
    }else if (waveGrowth < 0 && currentWavePoinCCX < 2 * waterWaveHeight *(1-_percent)){
        currentWavePoinCCX -= waveGrowth;
    }
...
-(void)setCurrentFirstWaveLayerPath{
    ...
    CGFloat y = currentWavePoinCCX;
    CGPathMoveToPoint(path, nil, 0, y);
    for (float x = 0.0f; x <=  waterWaveWidth ; x++) {
        // 正弦波浪公式
        y = waveAmplitude * sin(waveCycle * x + offsetX) + currentWavePoinCCX;
...
-(void)setCurrentSecondWaveLayerPath{
    ...
    for (float x = 0.0f; x <=  waterWaveWidth ; x++) {
        // 余弦波浪公式
        y = waveAmplitude * cos(waveCycle * x + offsetX) + currentWavePoinCCX;
        CGPathAddLineToPoint(path, nil, x, y);
    }
