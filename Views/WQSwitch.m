...
#define WQSwitchMaxHeight 31.0f
#define WQSwitchMinHeight 31.0f

#define WQSwitchMinWidth 51.0f

#define WQSwitchKnobSize 28.0f
...
- (void)layoutSubviews
{
    ...
    CGFloat margin = (CGRectGetHeight(self.bounds) - WQSwitchKnobSize) / 2.0;
    ...
    if (!self.isOn) {
        ...
        self.knobView.frame = CGRectMake(CGRectGetWidth(self.containerView.bounds) - margin - WQSwitchKnobSize,
                                         margin,
                                         WQSwitchKnobSize,
                                         WQSwitchKnobSize);
    }
    ...
    self.onLabel.frame = CGRectMake(lMargin,
                                    r - lHeight / 2.0,
                                    CGRectGetWidth(self.onContentView.bounds) - lMargin - WQSwitchKnobSize - 2 * margin,
                                    lHeight);
    
    self.offLabel.frame = CGRectMake(WQSwitchKnobSize + 2 * margin,
                                     r - lHeight / 2.0,
                                     CGRectGetWidth(self.onContentView.bounds) - lMargin - WQSwitchKnobSize - 2 * margin,
                                     lHeight);
}
...
- (void)commonInit
{
    ...
    _knobView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, WQSwitchKnobSize, WQSwitchKnobSize)];
    _knobView.backgroundColor = _thumbTintColor;
    _knobView.layer.cornerRadius = WQSwitchKnobSize / 2.0;
...
- (CGRect)roundRect:(CGRect)frameOrBounds
{
    ...
    if (newRect.size.height > WQSwitchKnobSize) {
        newRect.size.height = WQSwitchKnobSize;
    }
    ...
    if (newRect.size.width < WQSwitchKnobSize) {
        newRect.size.width = WQSwitchKnobSize;
    }
    ...
    switch (recognizer.state) {
            ...
            if (!self.isOn) {
                [UIView animateWithDuration:0.25
                                 animations:^{
                                     self.knobView.frame = CGRectMake(margin,
                                                                      margin,
                                                                      WQSwitchKnobSize + offset,
                                                                      WQSwitchKnobSize);
                                 }];
            } else {
                [UIView animateWithDuration:0.25
                                 animations:^{
                                     self.knobView.frame = CGRectMake(CGRectGetWidth(self.containerView.bounds) - margin - (WQSwitchKnobSize + offset),
                                                                      margin,
                                                                      WQSwitchKnobSize + offset,
                                                                      WQSwitchKnobSize);
                                 }];
        ...
        case UIGestureRecognizerStateFailed: {
            if (!self.isOn) {
                ...
                [UIView animateWithDuration:0.25
                                 animations:^{
                                     self.knobView.frame = CGRectMake(CGRectGetWidth(self.containerView.bounds) - WQSwitchKnobSize,
                                                                      margin,
                                                                      WQSwitchKnobSize,
                                                                      WQSwitchKnobSize);
                                 }];
